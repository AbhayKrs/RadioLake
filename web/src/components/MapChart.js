...
const MapChart = (props) => {
    ...
    const [mapData_Ready, setMapDataReady] = useState(false);
    ...
    useLayoutEffect(() => {
        ...
        map.events.on("globalpointerup", (ev) => {
            if (isPanning) {
                ...
                if (country_bound_stations.length > 0) {
                    ...
                    if (closest !== undefined) {
                        ...
                        console.log("pointSeries", dataItem);
                        if (dataItem) {
                            dataItem.set("fill", am5.color("#fff"));
                        }
                        playStation(closest);
                    }
                }
