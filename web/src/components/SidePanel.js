...
const SidePanel = (props) => {
    ...
    return (
        <div className="absolute z-30 bottom-1 left-1 z-40 max-w-[18em] w-full flex flex-col gap-2">
            {selectedCountry.length > 0 && <div className="flex flex-col items-center bg-amber-300 rounded-md">
                <div className={`flex flex-col items-center mb-2 px-2 w-full transition-colors duration-200 ${isOpen ? 'rounded-t-md' : 'rounded-md'}`}>
                    ...
                    <div className='flex flex-row justify-between items-end w-full'>
                        <div className='flex flex-col'>
                            ...
                            <p className="font-caviar font-bold text-neutral-800">{viewedStations.length} Stations</p>
                        </div>
                        <p className='font-josefin font-bold text-neutral-800'>{currentTime}</p>
                    </div>
                ...
                {isOpen && (
                    <>
                        ...
                        <ul style={{ maxHeight: "calc(100vh - 350px)" }} className='scrollbar flex flex-col gap-0.5 w-full h-full mt-2 px-2 overflow-y-auto'>
                            {viewedStations.map(itx => (
                                <li key={itx.id} onClick={() => handleStationChange(itx)} className={`rounded-md ${playingStation.data.stationuuid === itx.id ? 'bg-rose-600' : 'hover:bg-yellow-500/75'}`}>
                                    <p className={`font-roboto text-xs md:text-sm w-full p-1.5 cursor-pointer`}>{itx.name}</p>
                                </li>
                            ))}
                        </ul>
                    </>
            ...
            {playingStation.active && <div className='flex flex-col gap-2 bg-amber-300 p-2 rounded-md'>
                <div className="flex flex-row gap-2 items-center">
                    <img src={SiteLogo} className='h-12 w-12' />
                    <div>
